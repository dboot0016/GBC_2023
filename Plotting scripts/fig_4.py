...
VARS= 'FG_CO2', 'NBP'
UNIT= '[kg C s$^{-1}$]','[kg C s$^{-1}$]'
CONV= 1,12.01*1e-8,1e-3,1e6*28.966/44
DESCR='Ocean exchange','Land exchange'
...
n=0 # n = 0 for ocean uptake; n = 1 for land uptake
var=VARS[n]
unit=UNIT[n]
conv=CONV[n]
descr=DESCR[n]
...
load_var2 = xr.open_dataset(f'{data_snel2}/'+var+'_yr_'+RUN2+'_'+str(year_start)+'_'+str(2100)+'_0126.nc' )
...
#%% Regrid data
plt_var1 = (VAR1*area_gn).sum(['nlon','nlat'])
plt_var2 = (VAR2*area_gn).sum(['nlon','nlat'])

plt_var11 = np.cumsum(plt_var1)
plt_var12 = np.cumsum(plt_var2)

#%%
plt_var3 = (VAR3*area_gn).sum(['nlon','nlat'])
plt_var4 = (VAR4*area_gn).sum(['nlon','nlat'])

plt_var13 = np.cumsum(plt_var3)
plt_var14 = np.cumsum(plt_var4)

#%%   
plt_VAR1 = pd.DataFrame(plt_var11)
rolling_windows = plt_VAR1.rolling(tm)
PLT_VAR1=np.squeeze(rolling_windows.mean())

plt_VAR2 = pd.DataFrame(plt_var12)
rolling_windows = plt_VAR2.rolling(tm)
PLT_VAR2=np.squeeze(rolling_windows.mean())

plt_VAR1 = pd.DataFrame(plt_var13)
rolling_windows = plt_VAR1.rolling(tm)
PLT_VAR3=np.squeeze(rolling_windows.mean())

plt_VAR2 = pd.DataFrame(plt_var14)
rolling_windows = plt_VAR2.rolling(tm)
PLT_VAR4=np.squeeze(rolling_windows.mean())

#%%
PLT_VAR1 = np.array(PLT_VAR1) * (1e-12*365*86400)
PLT_VAR2 = np.array(PLT_VAR2) * (1e-12*365*86400)
PLT_VAR3 = np.array(PLT_VAR3) * (1e-12*365*86400)
PLT_VAR4 = np.array(PLT_VAR4) * (1e-12*365*86400)
...
ax.plot(t1,PLT_VAR2-PLT_VAR1,linewidth=LW,color='tab:green',linestyle='dashed',label='126')
plt.grid()
plt.xlabel('Time',fontsize=FS)
plt.ylabel('$\Delta$'+descr+' ' + unit2,fontsize=FS)
...
if save_fig == 'yes':
    plt.savefig(datadir+'fig_5d.png', format='png', dpi=quality,bbox_inches='tight')
...
plt.grid()
plt.xlabel('Time',fontsize=FS)
plt.ylabel(descr+' ' + unit2,fontsize=FS)
plt.xlim([year_start+5,year_end2])
plt.xticks(A,fontsize=FS-3)
plt.yticks(fontsize=FS-3)
...
if save_fig == 'yes':
    plt.savefig(datadir+'fig_5a.png', format='png', dpi=quality,bbox_inches='tight')

print((np.array(PLT_VAR4)-np.array(PLT_VAR3))[-1])
print((np.array(PLT_VAR2)-np.array(PLT_VAR1))[-1])   
...
ax = fig.add_axes([0.25,0.15,0.67,0.8])
ax.plot(t2,PLT_VAR4-PLT_VAR3,linewidth=LW,color='tab:green',label='585')
ax.plot(t1,PLT_VAR2-np.array(PLT_VAR1)[:],linewidth=LW,color='tab:green',linestyle='dashed',label='126')
plt.grid()
plt.xlabel('Time',fontsize=FS)
plt.ylabel('$\Delta$'+descr+' ' + unit2,fontsize=FS)
plt.xlim([year_start+5,year_end2])
...
if save_fig == 'yes':
    plt.savefig(datadir+'fig_5f.png', format='png', dpi=quality,bbox_inches='tight')
...
if save_fig == 'yes':
    plt.savefig(datadir+'fig_5c.png', format='png', dpi=quality,bbox_inches='tight')
